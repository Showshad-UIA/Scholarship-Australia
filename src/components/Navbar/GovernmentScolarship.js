...
const GovernmentScolarship = (d) => {
    const [govUniversity, setgovUniversity] = useState("");
    useEffect(() => {
		fetch("Government.json")
			.then((res) => res.json())
			.then((data) => {
				// console.log(data)
				setgovUniversity(data);
			});
	}, []);

    console.log(govUniversity);
    
    return (
        <>
					{govUniversity.map((data) => (
...
export default GovernmentScolarship;
